...
public class DUKPK2009_CBC {
	public static String ksn =  "";//36501A02001097E00001 trackksn
	
	
	public static String  getCardNumber(String track2) {
...
//		String ksn =  "aaaaa";
		...
		String deResultStr = parseByte2HexStr(TriDesDecryptionCBC(parseHexStr2Byte(dataKeyStr__), parseHexStr2Byte(track2)));
		System.out.println("origal_track2_data:"+track2);
		System.out.println("decode_track2_data:"+deResultStr);
		System.out.println("\n");
		
		// 国外卡号16位，国内卡号19位，根据第16位到19位判断当前卡为国外还是国内卡？
		for (int i = 0;i < 3;i++ ) {
			if (deResultStr.charAt(16+i) <= 'F' && deResultStr.charAt(16+i) >= 'A') {
				return deResultStr.substring(0,16);
			} else {
				return deResultStr.substring(0,19);
			}
		}
